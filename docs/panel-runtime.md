...
`CliPanelProcessLauncher` in `src/panel-process.ts` is the production `ProductionPanelProcessLauncher`. It starts controlled `pi --mode json` or `claude --print --output-format json` processes with each exact configured harness/model/reasoning profile, disables ambient Pi resources and context, and validates direct structured JSON output. Pi seats run the launching Pi package's JavaScript CLI through `process.execPath`, rather than invoking an npm command shim that Node cannot spawn directly on Windows. The process adapter has no custom versioned child IPC protocol.

Grilling accepts at least one complete usable Panel Answer after every configured seat terminates and records failed seats without retry or substitution. Review requires one complete usable report from every configured seat on the same exact head. A failed round reports every failed seat's harness, provider, model, reasoning level, and underlying process or output error instead of returning only the aggregate quorum failure. The authoritative Grilling Session or Review Session judges and applies the results; the Panel never does.
