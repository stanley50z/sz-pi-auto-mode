...
Both functions launch the three fixed `PANEL_EXECUTIONS` seats concurrently through a `PanelSeatLauncher`. Every seat receives the same deeply frozen context and a prompt that hides peers and prohibits tracker mutation, queue progression, nested subagents, merges, and human questions. The only seat tools are `read`, `grep`, `find`, and `ls` (mapped to `Read`, `Grep`, and `Glob` for Claude Code).
...
Grilling accepts at least one complete usable Panel Answer after all three seats terminate and records failed seats without retry or substitution. Review requires all three complete usable reports from the same exact head. The authoritative Grilling Session or Review Session judges and applies the results; the Panel never does.
