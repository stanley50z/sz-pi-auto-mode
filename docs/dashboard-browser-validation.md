...
3. Before the black-box launch, the deterministic proof server accepts its proof-only `EMPTY` stdin command; browser-harness verifies lifecycle `EMPTY`, zero candidates and totals across all four lanes, and captures the full empty run at all three approved viewports.
4. Browser-harness passes the exact Pi-emitted target to `new_tab`, renders Stage Lanes, opens the running `#50` Ticket Session drawer, and observes `npm test — live activity proof` in the live structured activity feed.
5. The browser invokes Graceful drain, captures `DRAINING` at all three approved viewports before the fixture settles, observes the explicit disconnected/stale state after dashboard shutdown, captures that state at all three viewports, and waits for the launched Pi process to exit successfully.
...
- Browser-harness recording: `C:\Users\13982\.config\browser-harness\agent-workspace\recordings\automode-dashboard-pr56-20260819-010632` (160 frames).
- Screenshot artifacts: `C:\Users\13982\AppData\Local\Temp\automode-dashboard-browser-proof-s_7z9r4x`.
...
- Launched Pi process for that run: PID `38860`, observed exit code `0` after browser drain.
...
| Full empty run | deterministic production-dashboard fixture | inspected | inspected | inspected |
