...
Auto-Implement uses deterministic branches and worktrees under `.worktree`; recovery resumes surviving sessions and reconstructs missing worktrees from branches. Auto-Grilling and Auto-Review use the controlled three-seat Panel runtime with identical immutable context, hidden peers, fixed execution profiles, and read-only advisory tools. Review workspaces are pinned to the exact pull-request head and checked for pushability before fixes. Successful review cleanup is gated on a completed merge.
