...
- extensions, prompt templates, appended prompts, themes, ambient skills, and default tools are disabled; only explicit package-owned Automode skill paths are loaded. Repository `AGENTS.md` or `CLAUDE.md` guidance remains available, while context files above the repository and normal-Pi context files stay excluded.
