...
openwiki:
  ...
  source_paths: [src/coordinator.ts, src/automode-main.ts, src/ticket-session.ts, src/ticket-session-main.ts, src/ticket-transcript.ts, src/github-tracker.ts, src/workspace.ts]
  symbols: [AutomodeCoordinator, AutomodeTicketSessionHost, createAssistantTranscript, ticketSessionActivityFromAgentEvent, GitHubTracker, WorkspaceManager]
...
- Child protocol, transcript projection, or termination: change `src/ticket-session.ts`, `src/ticket-session-main.ts`, or `src/ticket-transcript.ts`; run `test/ticket-session.test.ts` and `test/ticket-session-result.test.ts`. Protocol changes must be validated at the real child-process seam, not only by testing the internal transcript helper.
