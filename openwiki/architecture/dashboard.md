...
openwiki:
  ...
  symbols: [CoordinatorDashboard, DashboardProjection, DashboardCommand, HttpCoordinatorDashboard, createDashboardProjection, openLocalDashboard, createCanonicalTicketSessionPrompt, createAssistantTranscript, createAutomodeStatusCard, requestReturnToNormalPi, NestedSessionEvent]
  test_paths: [test/dashboard.test.ts, test/dashboard-ui.test.ts, test/dashboard-browser-proof-main.ts, test/dashboard-browser-walkthrough.py, test/coordinator.test.ts, test/main-session.test.ts, test/main-status-card.test.ts, test/ticket-session.test.ts]
  invariants: [The dashboard prefers loopback port 41738 and falls back to an operating-system-selected free loopback port., Tailscale exposure is optional and failures leave localhost access available., Only the exact owned Tailscale handler is removed., Waiting summaries remain visible in candidate reasons and latest structured activity., Run and Ticket Session elapsed values use validated Coordinator timestamps and matching session identity., Browser mutations require an allowed host same-origin checks CSRF and revision matching., Dashboard commands delegate to the Coordinator and never perform ticket work., Main Session /automode gracefully drains before returning to the original normal Pi session.]
...
The `/automode` Main Session starts a Coordinator-owned dashboard before tracker discovery, then opens the exact validated localhost URL in the operating system's default browser via `openLocalDashboard`. The browser is the comprehensive supervision surface; the Pi TUI remains a compact status and shutdown surface. The dashboard shows every open **Stage Candidate** in one of four precedence-selected lanes—Auto-Triage, Auto-Grilling, Auto-Implement, and Auto-Review—rather than only showing active Ticket Sessions. The product design is recorded in [`docs/automode-dashboard-design.md`](../../docs/automode-dashboard-design.md) and the architectural decision in [`docs/adr/0001-use-a-local-web-dashboard-for-coordinator-supervision.md`](../../docs/adr/0001-use-a-local-web-dashboard-for-coordinator-supervision.md).
...
`openLocalDashboard` is the launch-time browser boundary. It parses the dashboard URL and refuses anything other than `http:` on `127.0.0.1`; only after that check does it invoke the platform launcher—`rundll32.exe url.dll,FileProtocolHandler <url>` on Windows, `open <url>` on macOS, or `xdg-open <url>` on Linux and other platforms. The opener is awaited after `dashboard.start()` and before Coordinator discovery, so a browser-launch failure is part of startup rather than a background side effect. `StartAutomodeMainOptions.openDashboardInBrowser` provides the injected seam used by journey and Main Session tests to avoid launching a real browser.
...
`DashboardCommand` intentionally contains only `refresh`, `drain`, and `set-stage-state`. These control process-local Automation Stage Operating State (`ON`, `DRAINING`, `OFF`); disabling an active Stage drains it before `OFF`, and re-enabling requests a fresh eligibility scan. Operating State is not persisted in the **Automode Run Record**: a new process restores `ON`/`OFF` from the latest launch Automation Stage Configuration, which may have been replaced by a later `/automode` launch after the prior Coordinator stopped. The browser cannot force-stop individual sessions, reset retry budgets, mutate GitHub, prompt Ticket Sessions, or merge pull requests.
...
The dashboard starts before discovery, follows Coordinator lifecycle, and stops during final disposal. A port collision therefore fails startup before tracker reconciliation, snapshots, claims, or other workflow mutations. Run and Ticket Session timing is part of the projection: the run receives a Coordinator-clock `startedAt`, and durable session projections carry `startedAt` plus `endedAt` when settled. The HTTP envelope includes `serverTime`; the browser uses it to offset its local clock, so live elapsed values remain meaningful even when operator and Coordinator clocks differ. Recovered waiting or settled sessions retain their persisted timing, but timing is emitted only when the session ID still matches the durable record. The Main Session's async disposal waits for Coordinator stop, then stops the dashboard and releases the repository lock; this ordering prevents a live Coordinator from outliving its supervision surface. Browser drain and the Main Session `/automode` and `/drain` commands request only graceful drain; `/automode` additionally returns terminal ownership to the original normal Pi session, `/drain` exits after settling, and `/exit` is terminal-only and force-stops active Ticket Sessions before cleanup and exit. Automode does not intercept `Ctrl-C`, so Pi retains its default TUI behavior. During drain, enabled lanes transition to `DRAINING` when they have active work or directly to `OFF` when idle; re-enabling is a Coordinator operation that triggers a fresh scan.

## Change navigation and validation

- HTTP routes, security checks, Tailscale, event streams, browser opening, or command parsing: `src/dashboard.ts`; focused coverage is `test/dashboard.test.ts` and the startup ordering case in `test/main-session.test.ts`. `openLocalDashboard` must accept only the exact loopback HTTP URL; opening happens before Coordinator discovery.
- Projection shape, candidate labels, elapsed rendering, browser validation, or accessibility behavior: `src/dashboard-ui.ts`; use `test/dashboard-ui.test.ts` and the browser walkthrough/proof in `test/dashboard-browser-walkthrough.py` and `test/dashboard-browser-proof-main.ts`. Preserve timestamp validation, `endedAt`-requires-`startedAt`, and server-clock offset handling.
