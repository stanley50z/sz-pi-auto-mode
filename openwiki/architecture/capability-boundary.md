...
Project executable resources are accepted only when `trusted` is true, each path resolves inside the repository, and its `SKILL.md` exists. The capability attestation resolves the fixed ordinary Ticket Session profile (`openai-codex/gpt-5.6-sol`); it is separate from the process-local Main Session profile, which can resolve from the launching Pi model or normal `models.json`. The Automode Capability Attestation Session is used only for startup attestation and is disposed before the Main Session begins; it is not a Ticket Session.
