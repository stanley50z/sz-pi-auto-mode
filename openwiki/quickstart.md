...
| Change Ticket Session waiting, retained summaries, or work-contract fallback | [Coordinator and Ticket Sessions](architecture/coordinator.md) and [Skill catalog](skills/catalog.md) | `src/coordinator.ts`, `src/ticket-session.ts`, `skills/automode/implement/SKILL.md`, `skills/automode/code-review/SKILL.md` | `TicketSessionTerminalResult`, `AutomodeCoordinator`, `runAttempts`, `BookkeepingRecord` | `test/coordinator.test.ts`, `test/bundled-standards.test.ts`, `test/ticket-session.test.ts` | `npm run build && node --test dist/test/coordinator.test.js dist/test/bundled-standards.test.js dist/test/ticket-session.test.js` |
