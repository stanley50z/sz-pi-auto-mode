...
Auto-Implement and Auto-Review use the Agent Brief as a verification contract when present. If no templated Agent Brief exists, the complete issue discussion may serve as an equivalent contract when it settles behavior, public seams, verification, dependencies, and human gates; absent template headings alone are not a blocker. This rule belongs to `skills/automode/implement/SKILL.md` and `skills/automode/code-review/SKILL.md`, with structural coverage in `test/bundled-standards.test.ts`. It is separate from the Ticket Session waiting lifecycle described in the [Coordinator page](../architecture/coordinator.md).
