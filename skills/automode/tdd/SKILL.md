...
## Automode Stage customization

The linked issue/specification is the pre-agreed behavior contract. Use its stated public interface when present; otherwise choose the highest suitable public seam and proceed without requesting routine confirmation. Keep every RED/GREEN command under an external timeout, derive expected values independently, run focused checks throughout, run the complete applicable suite once, and finish through the real entrypoint with real data when available. Never weaken a test or add fallback/silent-failure behavior to obtain GREEN.
...
Test only at the issue/specification seam when it is stated. Otherwise identify
the highest suitable public interface and proceed autonomously. No test should
be written against an internal helper just because it is convenient.
...
- [ ] Read the issue/specification as the confirmed behavior contract
...
- [ ] Identify the stated public seam or choose the highest suitable public seam
- [ ] Identify opportunities for [deep modules](deep-modules.md)
- [ ] Design interfaces for [testability](interface-design.md)
- [ ] List the behaviors to test, not implementation steps
- [ ] Determine from the contract whether existing v1 behavior is preserved or replaced
...
- If you're adding a `try/catch`, default return, or `?? fallbackValue` to make
  a test green, stop and verify that the behavior is correct in production.
- If the planned approach keeps failing, stop the Ticket Session and preserve
  the failed approaches plus alternatives as diagnostics instead of silently switching.
...
- Never create `if v2 fails, fall back to v1` code unless the issue contract
  explicitly requires backward compatibility.
...
[ ] Public seam was pinned by the spec or autonomously chosen at the highest suitable boundary
