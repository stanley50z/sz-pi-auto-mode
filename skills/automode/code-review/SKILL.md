...
5. Do not run project-root restart scripts from the Review Session. After fresh merged proof, the Coordinator removes the worktree and same-repository local/remote head branches, fast-forwards the project root to the fetched remote default branch, and runs `stop.py` when present before `start.py` when `start.py` exists. It never deletes a fork branch. Post-merge finalization failure does not undo the merge.
6. Freshly verify the pull request is merged, then call `automode_ticket_result` exactly once with `status: "complete"`, a summary of the exact merged head and validation, and `finalDisposition` containing the complete `## Automode final validation` Markdown. Do not post that final disposition yourself. The controlled runtime appends exact whole-session token and cost totals and posts it after the full Review Session settles. Do not substitute prose for the structured result.
