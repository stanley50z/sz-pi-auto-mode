...
3. Call `automode_panel` once with `kind: "grilling"` and the complete structured Round Context. That controlled tool dispatches all configured seats concurrently with hidden peers. Do not launch panel processes or subagents through any other path.
...
After a fresh tracker read proves this grilling ticket is resolved and no longer eligible, call `automode_ticket_result` exactly once with `status: "complete"` and a concise evidence summary. Do not substitute prose for the structured result.
