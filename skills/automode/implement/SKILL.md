...
1. Read repository guidance, domain vocabulary, relevant ADRs, and the complete Agent Brief. Load the [Agent Brief verification contract](../../native/triage/AGENT-BRIEF.md) and proceed only when the brief passes its completeness check. Treat a missing predicate, uncovered acceptance criterion, unsatisfied dependency, or unresolved contradiction as an explicit failure.
...
6. Commit through the canonical `commit` skill, push the item branch, and open a non-draft pull request. The pull request must link and close the issue, summarize scope, and preserve every evidence item required by the Agent Brief.
